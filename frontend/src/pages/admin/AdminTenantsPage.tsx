...
export default function AdminTenantsPage() {
  ...
  async function handleCreateUser(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!organizationId) {
        throw new Error('Selecione uma empresa antes de criar o usuário.');
      }
      ...
      console.error(err);
      setCreateUserError(err instanceof Error ? err.message : 'Erro ao criar usuário.');
    } finally {
        ...
        <div className="rounded-2xl border border-divider-subtle bg-surface p-6 shadow-sm">
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-xl font-bold text-primary">Criar usuário com acesso real</h2>
              <p className="mt-1 text-sm text-secondary">
                O usuário será criado no Supabase Auth e vinculado ao profile da empresa escolhida.
              </p>
            </div>
          </div>
          ...
          <form className="grid grid-cols-1 gap-4 sm:grid-cols-2" onSubmit={handleCreateUser}>
            <div className="sm:col-span-2">
              ...
              <select
                className="w-full rounded-xl border border-divider-subtle bg-surface-input px-4 py-3 text-sm text-primary outline-none"
            ...
            <div className="sm:col-span-2">
              ...
              <p className="mt-2 text-xs text-secondary">
                Super admin terá acesso global ao sistema, mas continuará vinculado a uma empresa.
              </p>
            </div>

            {createUserError && (
              <div className="sm:col-span-2 rounded-xl border border-red-300/40 bg-red-500/10 px-4 py-3 text-sm text-red-600">
                {createUserError}
              </div>
            )}

            {createUserSuccess && (
              <div className="sm:col-span-2 rounded-xl border border-emerald-300/40 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-600">
                {createUserSuccess}
              </div>
            )}
            ...
            <div className="sm:col-span-2 flex items-center justify-end gap-3">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-70"
              >
