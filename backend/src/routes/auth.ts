...
export function createAuthRouter(options: { includeSuperAdmin: boolean }) {
  ...
  authRouter.post('/login', loginRateLimit, async (req, res, next) => {
    try {
      ...
      if (isProduction && config.ENABLE_TENANT_SUBDOMAIN && !tenantReq.tenantOrganizationId) {
        return res.status(400).json({
          error: 'tenant_required',
          message: 'Tenant subdomain is required to authenticate.',
        });
      }
      ...
      if (hasTenantMismatch(tenantReq, profile?.organization_id)) {
