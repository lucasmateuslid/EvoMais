...
import { AI_PROVIDERS, type ChatRequest } from '../types/ai.js';
...
function buildHistoryPreview(history: ChatRequest['history'] = []) {
...
function buildContextPreview(context: ChatRequest['context']) {
  return {
...
aiRouter.post('/chat', async (req, res, next) => {
  ...
  try {
    const payload = chatRequestSchema.parse(req.body) as ChatRequest;
