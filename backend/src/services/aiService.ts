...
import type { AIContext, ChatRequest, ChatResponse } from '../types/ai.js';
...
function buildCacheKey(message: string, history: ChatRequest['history'] = [], context: AIContext) {
  const payload = {
...
function buildFallbackResponse(request: ChatRequest) {
...
async function tryGeminiResponse(request: ChatRequest) {
