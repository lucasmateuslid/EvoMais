...
import type { AIContext, AIProvider, AIProviderPreference, ChatRequest, ChatResponse } from '../types/ai.js';
...
const normalizeHistory = (history: ChatRequest['history'] = []): string =>
  history.map(m => `${m.type}:${m.text}`).join('|').toLowerCase();
...
const isCRMQuestion = (question: string): boolean => {
  const normalized = question.toLowerCase();
  return [...crmIntentSet].some(term => normalized.includes(term));
};

const isMessageSuggestionQuestion = (question: string): boolean => {
  const normalized = question.toLowerCase();
  return [...messageSuggestionSet].some(term => normalized.includes(term));
};

const isSalesAnalysisQuestion = (question: string): boolean => {
  const normalized = question.toLowerCase();
  return [...salesAnalysisSet].some(term => normalized.includes(term));
};

const shouldClassifyAggressiveness = (question: string): boolean => {
  const normalized = question.toLowerCase();
  return isSalesAnalysisQuestion(question) || [...aggressivenessSet].some(term => normalized.includes(term));
};

const hasWeakPermissionLanguage = (text: string): boolean =>
  WEAK_PERMISSION_PATTERNS.some(pattern => pattern.test(text));

const hasAggressivenessLabel = (text: string): boolean =>
  /(nível\s*[1-4]|nivel\s*[1-4]|\bn[1-4]\b)/i.test(text);

// ─────────────────────────────────────────────
// Templates de mensagens AGRESSIVAS (nova versão)
// ─────────────────────────────────────────────

function buildAuthorityMessageTemplates(request: ChatRequest): string {
...
function buildCommonContext(request: ChatRequest): string {
...
const buildOpenAICompatibleMessages = (request: ChatRequest) => [
  { role: 'system' as const, content: buildSystemPrompt(request) },
  ...(request.history || []).slice(-15).map(m => ({ // -15 para maior agilidade (menos tokens)
...
const buildGeminiContents = (request: ChatRequest) => [
...
const buildAnthropicMessages = (request: ChatRequest) =>
  (request.history || []).slice(-15).map(m => ({
    role: m.type === 'user' ? ('user' as const) : ('assistant' as const),
    content: m.text,
  })).concat({ role: 'user' as const, content: request.message });

// ─────────────────────────────────────────────
// Normalização e extração de texto (otimizada)
// ─────────────────────────────────────────────

const normalizeProviderText = (provider: AIProvider | 'fallback', text: string | null | undefined): string => {
  const parsed = providerTextSchema.safeParse(text?.trim());
  if (!parsed.success) throw new Error(`${provider} returned invalid/empty response`);
  return parsed.data;
};

const isOpenAICompatibleTruncated = (finishReason?: string | null) =>
  finishReason === 'length' || finishReason === 'max_tokens';

const extractOpenAICompatibleResult = (payload: unknown, provider: AIProvider): ProviderResult => {
...
const extractGeminiResult = (payload: unknown): ProviderResult => {
...
const providerInvokers: Record<AIProvider, (request: ChatRequest) => Promise<ProviderResult | null>> = {
...
const resolveProviderOrder = (preference: AIProviderPreference = 'auto'): AIProvider[] => {
  const configured = config.AI_PROVIDER_ORDER;
  return preference === 'auto' ? configured : [preference, ...configured.filter(p => p !== preference)];
};

function buildContinuationRequest(request: ChatRequest, partialText: string): ChatRequest {
...
function buildFallbackResponse(request: ChatRequest): string {
...
function enforceMessageSuggestionStyle(request: ChatRequest, text: string): string {
  if (!isMessageSuggestionQuestion(request.message)) return text;
