...
export function ConnectionsModal({ isOpen, onClose }: ConnectionsModalProps) {
  ...
  useEffect(() => {
    if (profile?.organization_id && isOpen) {
      fetchConnections();
    }
        ...
        setConnections([
          {
            ...
            name: 'Consultora Joyce',
            phone: '84987596134',
            instance_name: 'lock-protecao-veicular-joyce',
            status: 'connected',
          ...
          },
          {
            id: '2',
            organization_id: profile?.organization_id || '',
            name: 'Consultora Melry Câmara',
            phone: '84987223259',
            instance_name: 'lock-protecao-veicular-melry',
            status: 'connected',
            api_provider: 'whatsmeow',
            created_at: new Date().toISOString()
          },
          {
            id: '3',
            ...
            name: 'Vendas Gerais',
            phone: '84981331131',
            instance_name: 'lock-protecao-veicular-vendas',
            status: 'disconnected',
            api_provider: 'evolution',
            created_at: new Date().toISOString()
          }
  ...
  const handleCreateConnection = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Erro ao criar conexão. Verifique se a tabela "connections" existe no Supabase.');
    } finally {
  ...
  const handleDeleteConnection = async (id: string) => {
    ...
    try {
      ...
      // Optimistic delete for mock data
      setConnections(connections.filter(c => c.id !== id));
    }
  };
  ...
  const updateConnectionStatus = async (id: string, status: 'connected' | 'disconnected' | 'connecting') => {
    try {
      ...
      if (error) throw error;
      setConnections(connections.map(c => c.id === id ? { ...c, status } : c));
    } catch (error) {
